...
> ⚠️ **Esqueleto** — ficaram apenas os **erros globais** (válidos para qualquer
> rota). Cada domínio do sistema registra a sua própria tabela abaixo conforme os
> N3 técnicos forem gerados.
...
> Erros específicos do domínio **Clientes** (feature set Gestão de Clientes),
> registrados a partir dos N3 técnicos. Acrescente novas seções por domínio
> conforme forem especificados.

| Código de erro | HTTP | Situação |
|---|---|---|
| `CLIENTE_NOT_FOUND` | 404 | Cliente não existe ou foi excluído (logicamente) |
| `CLIENTE_CPF_DUPLICADO` | 422 | Já existe cliente ativo cadastrado com o CPF informado |
...
**Regras**:
- Prefixo = nome do domínio em maiúsculas (ex.: `CADASTRO_`, `AUTH_`)
